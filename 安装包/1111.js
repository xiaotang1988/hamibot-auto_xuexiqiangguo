...
var { four_player_battle_tanking } = hamibot.env;
...
function do_contest(renshu) {
    ...
    while (true) {
        ...
        if (renshu == 0) {
            for (let k = 0; k < 11; k++) {
                toast("首局已挂起。请勿点击任何选项，不要作答！不要作答！！不要作答！！！");
                sleep(200);
            }
            num++;
            text("继续挑战").waitFor();
...
function get_tiku_by_http(link) {
    // 通过访问gitcode的数据直链
    if (!link) {link = "https://gitcode.net/m0_64980826/songge_tiku/-/raw/master/tiku_json.txt"}
...
if (!finish_list[6] && four_players_scored < 3) {
    ...
    entry_model(10);
    var num_2 = 0;
    if (four_player_battle_tanking == "yes") { do_contest(0); }
    do_contest(4);
    num_2++;
    do_contest(4);
}
